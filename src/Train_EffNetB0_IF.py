# -*- coding: utf-8 -*-
"""
Script for training the EfficientNet-B0 with the Intermediate Fusion (IF) on the
new dataset, derived from the Damage Level Classification Challenge (PHI-Net),
...
with tf.device('CPU/:0'):
    ...
    training_set = Dataset.from_tensor_slices(((X[:-N_T],W[:-N_T]),y[:-N_T]))
    training_set = training_set.batch(N_b)

    validation_set = Dataset.from_tensor_slices(((X[-N_T:],W[-N_T:]),y[-N_T:]))
...
net = models.build_IF_model(num_classes=4, LR=LR)
model_name = 'EfficientNetB0_IF'
# net.summary()
